...
SCHEMA_SQL_PATH = Path(__file__).resolve().parents[2] / "config" / "ppi_schema.sql"

PPI_FIELDS: Sequence[str] = (
    "period_date",
    "period_label",
    "current_index",
    "yoy_change",
...
class MacroPpiDAO(PostgresDAOBase):
    """Persistence helper for PPI history."""

    _conflict_keys: Sequence[str] = ("period_date",)

    def __init__(self, config: PostgresSettings, table_name: Optional[str] = None) -> None:
        super().__init__(config=config)
        self._table_name = table_name or getattr(config, "ppi_table", "macro_ppi")
    ...
    def list_entries(self, *, limit: int = 200, offset: int = 0) -> Dict[str, object]:
        ...
        for period_date, period_label, current_index, yoy_change, cumulative_index, updated_at in rows:
            items.append(
                {
...
__all__ = ["MacroPpiDAO", "PPI_FIELDS"]
