"""DAO for caching Tonghuashun concept directory."""
...
SCHEMA_SQL_PATH = Path(__file__).resolve().parents[2] / "config" / "concept_directory_schema.sql"


class ConceptDirectoryDAO(PostgresDAOBase):
    """Persist concept name/code mappings."""

    def __init__(self, config: PostgresSettings) -> None:
        super().__init__(config=config)
        self._table = getattr(config, "concept_directory_table", "concept_directory")
        ...
        with self.connect() as conn:
            ...
            with conn.cursor() as cur:
                ...
                if mapping:
                    insert = sql.SQL(
                        """
                        INSERT INTO {table} (concept_name, concept_code, updated_at)
    ...
    def list_entries(self) -> List[Dict[str, str]]:
        with self.connect() as conn:
            ...
            with conn.cursor() as cur:
                ...
                rows = cur.fetchall()
        return [{"concept_name": row[0], "concept_code": row[1]} for row in rows]
