...
SCHEMA_SQL_PATH = Path(__file__).resolve().parents[2] / "config" / "shibor_schema.sql"

SHIBOR_FIELDS: Sequence[str] = (
    "period_date",
    "period_label",
    "on_rate",
    "rate_1w",
...
class MacroShiborDAO(PostgresDAOBase):
    """Persistence helper for SHIBOR quotes."""

    _conflict_keys: Sequence[str] = ("period_date",)

    def __init__(self, config: PostgresSettings, table_name: Optional[str] = None) -> None:
        super().__init__(config=config)
        self._table_name = table_name or getattr(config, "shibor_table", "macro_shibor")
...
__all__ = ["MacroShiborDAO", "SHIBOR_FIELDS"]
