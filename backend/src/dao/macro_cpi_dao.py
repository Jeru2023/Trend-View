"""
DAO for macro CPI monthly statistics.
"""
...
SCHEMA_SQL_PATH = Path(__file__).resolve().parents[2] / "config" / "cpi_monthly_schema.sql"

CPI_MONTHLY_FIELDS: Sequence[str] = (
    "period_date",
    "period_label",
    "actual_value",
    "forecast_value",
...
class MacroCpiDAO(PostgresDAOBase):
    """Persistence helper for CPI monthly data."""

    _conflict_keys: Sequence[str] = ("period_date",)

    def __init__(self, config: PostgresSettings, table_name: Optional[str] = None) -> None:
        super().__init__(config=config)
        self._table_name = table_name or getattr(config, "cpi_table", "macro_cpi_monthly")
...
__all__ = ["MacroCpiDAO", "CPI_MONTHLY_FIELDS"]
