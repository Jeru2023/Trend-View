"""DAO for concept insight summaries."""
...
SCHEMA_SQL_PATH = Path(__file__).resolve().parents[2] / "config" / "concept_insights_schema.sql"

_TABLE_INIT_LOCK = Lock()
_INITIALISED_TABLES: Set[Tuple[str, str]] = set()


class ConceptInsightDAO(PostgresDAOBase):
    """Persistence helper for concept insight reasoning snapshots."""

    def __init__(self, config: PostgresSettings, table_name: Optional[str] = None) -> None:
        super().__init__(config=config)
        self._table_name = table_name or getattr(config, "concept_insight_table", "concept_insights")
    ...
    def latest_summary(self) -> Optional[Dict[str, object]]:
        with self.connect() as conn:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    sql.SQL(
                        "SELECT summary_id, generated_at, window_start, window_end, concept_count, summary_snapshot, "
                        "summary_json, raw_response, referenced_concepts, referenced_articles, prompt_tokens, "
        ...
        record["referenced_concepts"] = self._decode_json(record.get("referenced_concepts"))
