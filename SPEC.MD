...
## Cash Flow Sync Enhancements (2025-11-16)
- Persist the core 18 cash-flow metrics from Tushare (operating, investing, financing, net change, cash balances, free cash flow) for all tracked stocks and expose them via `/financial/cashflow`.
- Surface a Finance → Cash Flow page plus a Control Panel card so operators can search the dataset and manually trigger the sync job.
- When adding new Control Panel cards, ensure the JS is defensive (null-safe DOM lookups) and bump the script query string so browsers download the updated bundle immediately.
- Mirror the same pipeline for balance sheet data (21 core asset/liability/equity metrics) with `/financial/balance-sheet`, a dedicated Finance page, and a Control Panel trigger wired into the monitor.

## Indicator Screening Real-Time Refactor Plan (2025-11-11)
1. **Stage 1 – Persist local indicators in the shared snapshot (Complete)**  
   - After daily trade and big-deal fund-flow syncs finish, immediately call `sync_indicator_screening` for 爆量启动 and 大单流入 so both metrics land in `indicator_screening`.  
   - No schema delta; goal is to keep third-party and self-computed indicators in one table for downstream joins.
2. **Stage 2 – Switch screening queries to database joins (In Progress)**  
   - Use `daily_trade` (latest trading date) as the base dataset to guarantee the full A-share universe, LEFT JOIN to `indicator_screening` for the indicators requested in filters, and include `daily_trade_metric`/`fundamental` tables as needed for existing knobs.  
   - Implement pagination in SQL (limit/offset) so the endpoint can scan >500 rows. Preserve current response schema but ensure deduped stock codes.  
   - Add targeted DAO tests covering “no indicator filters”, “only second-row filters”, and the two local indicators.
3. **Stage 3 – Expand indicator persistence coverage**  
   - Move remaining third-party-only filters (e.g., 连板、爆量资金) into the same `indicator_screening` ingestion so UI filters always read from the DB snapshot.  
   - Introduce a lightweight metadata table enumerating indicator codes → latest trade date + data provenance to simplify sync orchestration.
4. **Stage 4 – Real-time enrichments & performance**  
   - For indicators that rely on intra-day calculations, add incremental writers that update `indicator_screening` in near real time (e.g., via `sync_indicator_screening` partial updates or materialized views).  
   - Add covering indexes across `indicator_screening (trade_date, indicator_code, stock_code)` and `daily_trade (trade_date, stock_code)` plus composite indexes for frequent filters (turnover, percent change).  
   - Implement caching / snapshot versioning so the frontend knows when datasets refresh mid-session.
5. **Stage 5 – Frontend alignment & monitoring**  
   - Update the indicator screening UI to reflect the new pagination model, show the total universe count, and enforce deduped stock cards.  
   - Add monitoring hooks/logging around the job chain so deadlocks/long-running indicator syncs are surfaced immediately.
...
- 2025-11-14 (CHANGE backend/frontend-201): Retired the HSGT (陆港通) fund-flow module entirely once the upstream data feed was discontinued; removed its tables/DAO/services, control-panel job, and dedicated UI.
...
- 2025-10-31 (FEATURE backend/frontend-028): Added global index ingestion via Yahoo Finance daily history (with a Tushare FTSE A50 fallback), control-panel automation, “外围” navigation group, language-aware refresh button, and a dedicated global indices dashboard.
- 2025-11-14 (CHANGE backend/frontend-090): Consolidated global index storage into the normalized history table, removed the legacy snapshot table, refreshed the control-panel copy, and enforced a one-year window + FTSE fallback alignment across backend/front-end.
- 2025-11-14 (UPDATE backend/frontend-091): Peripheral insight pipeline now uses DeepSeek Reasoner, stores JSON-safe metrics for historical replay, exposes `/peripheral/insights/history`, and the LLM prompt explicitly拆分数据源（含美联储声明）及带时间戳。
...
- Fund-flow aggregate job now chains industry → concept → individual → margin account → market fund flow → big deal.
...
- New page `frontend/public/macro-insight.html/js` renders:
...
- Exposed via `/market/overview` purely as a structured snapshot (no built-in LLM step).
...
1. `build_market_overview_payload` pulls:
   ...
   - Recent 10-day slices for market fund flow and margin account tables.
...
### Operational Notes
- Designed as a read-only API; callers that need reasoning can stitch their own prompts using the payload.
...
- 结果仍存入 `market_insights`（`backend/config/market_insights_schema.sql`），记录 prompt/response usage、结构化 summary、newsSignals。
...
4. 解析到的 JSON 与原始响应一起写入 `market_insights`，DAO 仍做幂等 upsert；若返回不合法 JSON 会保留 raw text 以便排查。
