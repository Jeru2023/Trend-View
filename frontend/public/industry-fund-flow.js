const translations = getTranslations("industryFundFlow");
const API_BASE =
  window.API_BASE_URL ||
...
async function fetchSymbol(symbol) {
  ...
  const response = await fetch(`${API_BASE}/fund-flow/industry?${params.toString()}`);
  if (!response.ok) {
    ...
    const results = await Promise.all(
      TABS.map(async (tab) => {
        try {
          ...
          console.error(`Failed to load industry fund flow for ${tab.value}`, error);
...
function renderActiveTab() {
  ...
  const page = Math.min(getActivePage(), Math.max(1, Math.ceil(total / STATE.limit) || 1));
  setActivePage(page);
...
function renderTable(items) {
  ...
  items.forEach((item) => {
    ...
    const industryIndex = formatNumber(item.industryIndex, {
    ...
    row.innerHTML = `
      <td>
        <div class="metric-stack">
          ...
          <div class="metric-row">
            <span class="metric-row__label">${dict.colIndustry}</span>
            <span class="metric-row__value metric-row__value--accent">${industry}</span>
          </div>
          <div class="metric-row">
      ...
      <td>
        <div class="metric-stack">
          <div class="metric-row">
            ...
            <span class="metric-row__value">${industryIndex}</span>
          </div>
          <div class="metric-row">
